import type { ModelInfo, ModelCapabilities, ModelPricing } from '../core/interfaces';
...
export class LiteLLMModelManager {
	 ...
	 */
	async fetchModelData(forceRefresh = false): Promise<Record<string, LiteLLMModel>> {
		const now = Date.now();

		// Return cached data if still valid
		if (!forceRefresh && this.modelData && now - this.lastFetch < this.CACHE_TTL) {
	 ...
	 */
	async getModelInfo(modelId: string, provider?: string): Promise<ModelInfo | null> {
	...
	async findModelsByCapabilities(capabilities: (keyof ModelCapabilities)[]): Promise<ModelInfo[]> {
		...
		for (const [modelId, modelData] of Object.entries(data)) {
			...
			const hasAllCapabilities = capabilities.every((capability) => {
				switch (capability) {
					...
					case 'audio':
						return (
							modelData.supports_audio_input ||
							modelData.max_audio_per_prompt ||
							modelData.supports_audio_output
						);
					case 'reasoning':
					...
					case 'video':
						return modelData.supports_video_input || modelData.max_videos_per_prompt;
					case 'embeddings':
		...
		return models.sort((a, b) => (a.pricing?.inputTokens || 0) - (b.pricing?.inputTokens || 0));
	}
	...
	async getCheapestModel(
		...
		const modelsWithPricing = compatibleModels.filter((m) => m.pricing?.inputTokens);

		if (modelsWithPricing.length === 0) return null;

		return modelsWithPricing.sort((a, b) => a.pricing!.inputTokens - b.pricing!.inputTokens)[0];
	}

	/**
	 * Convert LiteLLM model data to our ModelInfo format
	 */
	private convertToModelInfo(modelId: string, data: LiteLLMModel): ModelInfo | null {
		...
		const capabilities: ModelCapabilities = {
			...
			audio:
				...
				data.max_audio_per_prompt != null ||
				data.supports_audio_output ||
				false,
			embeddings: data.mode === 'embedding',
			reasoning: data.supports_reasoning || false,
			video: data.supports_video_input || data.max_videos_per_prompt != null || false,
	...
	private extractProvider(modelId: string, litellmProvider: string): string {
		// Map LiteLLM providers to our provider names - prioritize the actual provider
		const providerMap: Record<string, string> = {
			openai: 'openai',
			anthropic: 'anthropic',
			gemini: 'gemini',
			'vertex_ai-language-models': 'gemini', // Map Vertex AI models to use Gemini provider
			mistral: 'mistral',
			cohere: 'cohere',
			openrouter: 'openrouter',
		};

		// If we have a known litellm provider, use that mapping
		if (providerMap[litellmProvider]) {
			return providerMap[litellmProvider];
		}

		// Handle OpenRouter format (legacy support)
		if (modelId.startsWith('openrouter/')) {
			return 'openrouter';
		}

		// Handle provider/model format - only for unknown providers
		if (modelId.includes('/')) {
			const parts = modelId.split('/');
			const prefix = parts[0];

			// Map provider prefixes to standard names (only for direct API access)
		...
		// Fallback to original litellm provider
