...
export class OpenAIProvider implements ProviderAdapter {
    ...
    async generateCompletion(request: CompletionRequest): Promise<CompletionResponse> {
        try {
            ...
            if (request.responseFormat) {
                openAIRequest.response_format = this.convertResponseFormat(request.responseFormat);
            }
    ...
    async getModel(modelId: string): Promise<ModelInfo | null> {
        return MODEL_DATA.find(m => m.id === modelId && m.provider === this.name) || null;
    }
     ...
     */
    private prepareRequest(request: CompletionRequest): Omit<OpenAI.Chat.ChatCompletionCreateParams, 'stream'> {
        const openAIRequest: Omit<OpenAI.Chat.ChatCompletionCreateParams, 'stream'> = {
            model: request.model,
            messages: this.convertMessages(request.messages),
            temperature: request.temperature,
            max_tokens: request.maxTokens,
