/**
 * --- 03A. TOOL USAGE - OPENAI ---
 *
 * This example demonstrates how to use tools with OpenAI's GPT models.
 ...
 * 2.  Sending a completion request with the defined tool to OpenAI model.
 ...
 *
 * bun run examples/03a-tool-usage-openai.ts
 */

import { ModelManager, OpenAIProvider, ToolDefinition, ToolCall, Message } from "../src/index";
...
async function main() {
    console.log("--- 03A. TOOL USAGE - OPENAI ---");
    ...
    try {
        ...
        if (useStreaming) {
            ...
            // Collect the full response from streaming using proper OpenAI pattern
            let fullContent = "";
            let usage: any;
            const finalToolCalls: { [index: number]: any } = {};
            
            for await (const chunk of openai.streamCompletion(initialRequest)) {
                if (chunk.delta) {
                    fullContent += chunk.delta;
                    process.stdout.write(chunk.delta);
                }
                
                // Accumulate tool call deltas (following OpenAI documentation pattern)
                if (chunk.toolCallDeltas) {
                    for (const toolCallDelta of chunk.toolCallDeltas) {
                        ...
                        // Update id and name if they're provided (usually in first delta)
                        if (toolCallDelta.id) {
            ...
            initialResponse = await openai.generateCompletion(initialRequest);
        ...
        if (useStreaming) {
            ...
            for await (const chunk of openai.streamCompletion(followUpRequest)) {
                if (chunk.delta) {
                    process.stdout.write(chunk.delta);
                }
            ...
            const finalResponse = await openai.generateCompletion(followUpRequest);
