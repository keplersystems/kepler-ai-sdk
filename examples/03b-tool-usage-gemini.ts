 ...
 * 2.  Sending a completion request with the defined tool to Gemini model.
 ...
 *
 * bun run examples/03b-tool-usage-gemini.ts
 */

import { ModelManager, GeminiProvider, ToolDefinition, ToolCall, Message } from "../src/index";
...
async function main() {
    console.log("--- 03B. TOOL USAGE - GEMINI ---");
    ...
    try {
        ...
        if (useStreaming) {
            ...
            for await (const chunk of gemini.streamCompletion(initialRequest)) {
                if (chunk.delta) {
            ...
            initialResponse = await gemini.generateCompletion(initialRequest);
        ...
        if (useStreaming) {
            ...
            for await (const chunk of gemini.streamCompletion(followUpRequest)) {
                if (chunk.delta) {
                    process.stdout.write(chunk.delta);
                }
            ...
            const finalResponse = await gemini.generateCompletion(followUpRequest);
